...
export const CATEGORY_NAME_TO_IDX = CATEGORY_IDX_TO_NAME.reduce(
  (prev, curr, idx) => {
    prev[curr] = idx;
    return prev;
  },
...
export type BulkyShopItemRecord<T extends BulkyShopItem = BulkyShopItem> = Map<
  `${T["type"]}_${T["tier"]}`,
  T
>;
...
export type BulkyBazaarItemRecord<T extends BulkyBazaarItem = BulkyBazaarItem> =
  Map<`${T["type"]}_${T["tier"]}`, T>;
...
export type BulkyFilter<T extends BulkyFilterField = BulkyFilterField> = {
  ...
  regex?: string;
  fullSets?: boolean;
  delirious?: boolean; // For 8mod maps.
  originator?: boolean; // For 8mod maps.
...
export type ComputedBulkyFilterStore = {
  ...
  removeFilterField: <T extends BulkyFilter["uuid"]>(
    uuid: T,
    idx: number
  ) => void;
};
 ...
 * })
 */
export type BulkyItemOverrideRecord<T extends BulkyShopItem = BulkyShopItem> =
  Map<`${T["type"]}_${T["tier"]}`, BulkyItemOverrideInstance<T>>;
