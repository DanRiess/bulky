...
const AUTO_SYNC_INTERVAL = parseInt(
  import.meta.env.VITE_OFFER_AUTOSYNC_INTERVAL ?? 600000
);
...
export const useShopStore = defineStore("shopStore", () => {
  ...
  // GETTERS
  const maximumOffersReached = computed(
    () =>
      offers.value.length >= parseInt(import.meta.env.VITE_MAXIMUM_OFFER_AMOUNT)
  );
  ...
  async function regularUpdate() {
    for await (const offer of offers.value) {
      ...
      const timeSinceLastUpdate =
        Date.now() - (offer.timestamps[offer.timestamps.length - 1] ?? 0);
      ...
      if (
        offer.autoSync &&
        timeSinceLastUpdate > AUTO_SYNC_INTERVAL &&
        offer.league === configStore.config.league
      ) {
  ...
  function generateOffer(options: {
    ...
    options.itemRecord.forEach((item) => {
      ...
      if (computedMultiplier) {
        const itemMultiplier =
          toValue(item.priceOverride) / toValue(item.price);
    ...
    const fullPrice = useAggregateItemPrice(
      options.itemRecord,
  ...
  function updateOffer(offer: BulkyShopOffer, itemRecord: BulkyShopItemRecord) {
    ...
    offer.chaosPerDiv =
      ninjaStore.chaosPerDiv > 0 ? ninjaStore.chaosPerDiv : offer.chaosPerDiv;
  ...
  async function putOffer(offer: BulkyShopOffer) {
    ...
    const offerIdx = offers.value.findIndex(
      (oldOffer) => oldOffer.uuid === offer.uuid
    );
  ...
  async function recomputeOffer(
    ...
    const stashTabs = offer.stashTabIds
      .map((id) => stashStore.getStashTabById(id))
      .filter(Boolean);

    // Get the PoeItems from Idb and filter by the offer's category.
    const { filteredItemsByCategory, updateItemsByStash } =
      usePoeItems(stashTabs);
    const poeItems = filteredItemsByCategory(offer.category);

    // Generate BulkyItems from the PoeItems with prices and overrides.
    const { prices, loadingStatus: ninjaLoadingStatus } = usePoeNinja(
      offer.category
    );
    const { itemOverrides } = useItemOverrides(offer.category);
    const { items: itemRecord } = useBulkyItems(
      poeItems,
      prices,
      itemOverrides,
      offer.category
    ...
    while (
      ninjaLoadingStatus.value === "IDLE" ||
      ninjaLoadingStatus.value === "PENDING"
    ) {
    ...
    filteredItemRecord.value.forEach((item) => {
      ...
      // They have to be split into their specific faction based on highest faction price.
        ...
        const logbooks = replaceExpeditionLogbook(
          item as ShopExpeditionItem,
          itemOverrides
        );
        Object.values(logbooks).forEach((logbook) => {
          if (
            toValue(logbook.price) === 0 &&
            toValue(logbook.priceOverride) === 0
          )
            return;
    ...
    if (
      options.blockIfLowValue &&
      fullPrice.value < configStore.config.shop.autoUploadPriceFloor
    ) {
  ...
  function generatePutDto(
    ...
    const minimumBuyout =
      offer.minimumBuyout.divine * offer.chaosPerDiv +
    ...
    return {
      ...
      items: offer.items
        .map((item) =>
          BULKY_TRANSFORM.bulkyItemToBazaarItemDto(item, offer.multiplier ?? 1)
        )
        .filter(Boolean),
    };
  }
