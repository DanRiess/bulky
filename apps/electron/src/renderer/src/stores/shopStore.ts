...
export const useShopStore = defineStore('shopStore', () => {
	...
	function generateOffer(options: {
		...
		if (!authStore.profile?.name && import.meta.env.VITE_NO_ATTACH_MODE !== 'true') {
		...
		options.itemRecord.forEach(item => {
			...
			if (appStateStore.selectedCategory === 'MAP_8_MOD' && item.category === 'MAP_8_MOD') {
				...
				const perItemAttributes = item.perItemAttributes.filter(pia => {
					...
					if (pia.properties.delirious && pia.properties.originator && priceOverride.originatorDeli === 0) return false
					else if (pia.properties.delirious && priceOverride.baseDeli === 0) return false
					else if (pia.properties.originator && priceOverride.originator === 0) return false
					else if (!pia.properties.delirious && !pia.properties.originator && priceOverride.base === 0) return false
					return true
				})
