...
export const useMap8ModOfferStore = defineStore("Map8ModOfferStore", () => {
  ...
  function putOffer(dto: BulkyBazaarOfferDto) {
    ...
    const items = dto.items
      .map((item) => BULKY_MAPS.generateBazaarMap8ModItemFromDto(item))
      .filter(notEmpty);
    if (!items) return;
  ...
  function calculateBaseItemPrice(item: BazaarMap8Mod, filter: BulkyFilter) {
    ...
    const regexes = filter.regex
      ? BULKY_REGEX.computeRegexesFromString(filter.regex)
      : [];
    ...
    const quantityRegexes = regexes.filter((regex) =>
      regex.toString().match(/m q.*/i)
    );
    const packSizeRegexes = regexes.filter((regex) =>
      regex.toString().match(/iz.*([1-9]).*%/i)
    );
    const rarityRegexes = regexes.filter((regex) =>
      regex.toString().match(/y: \([(n|m|r)]*\)/i)
    );
    const avoidRegexes = regexes.filter((regex) =>
    ...
    if (quantityRegexes.length > 0) {
      ...
      const sortedItemQuantPricing = item.regex.quantityRegex?.toSorted(
        (a, b) => a[0] - b[0]
      );
    ...
    if (packSizeRegexes.length > 0) {
      // Sort the item's quant prices by quantity
      const sortedItemPacksizePricing = item.regex.packsizeRegex?.toSorted(
        (a, b) => a[0] - b[0]
      );
    ...
    if (avoidRegexes.length > 0) {
      if (item.regex.avoidRegex === undefined) {
        throw new UserError({
          code: "regex_unsupported",
          message:
            "This item does not support regexes with modifiers to avoid.",
        });
      }
...
if (import.meta.hot) {
  import.meta.hot.accept(
    acceptHMRUpdate(useMap8ModOfferStore, import.meta.hot)
  );
}
