...
import { MAP_MODIFIERS, MAP_TIER, MAP_TIER_IDX_TO_NAME, MAP_TYPE, MAP_TYPE_IDX_TO_NAME } from './map.const'
...
function generateBazaarMap8ModItemFromDto(item: BulkyBazaarItemDto): BazaarMap8Mod | undefined {
	...
	const perItemAttributes: Map8ModPerItemAttributes[] | undefined = item.pia
		?.map((attrs): Map8ModPerItemAttributes | undefined => {
			if (!attrs.prc || !attrs.mods || !attrs.props || !attrs.props.iQnt || !attrs.props.pckSz) return
...
function createFullRegexFromModifier(modifier: string) {
	...
	const regexPattern = modifier.replace('+', '\\+').replace(numberRegex, '.*').replace('\n', '|')
...
function getShop8ModBasePrice(item: ShopMap8Mod) {
	...
	return item.perItemAttributes.reduce((prev, individualItem) => {
		if (individualItem.properties.delirious && individualItem.properties.originator) {
...
function getIndividualPriceForDto(attributes: PerItemAttributes, item: BulkyShopItem) {
