...
import { BULKY_DELIRIUM_ORBS } from './deliriumOrb.transformers'
...
export const useDeliriumOrbOfferStore = defineStore('deliriumOrbOfferStore', () => {
	...
	function putOffer(dto: BulkyBazaarOfferDto) {
		...
		const items = conformBinaryListingItems<BazaarDeliriumOrb>(
			dto.items,
			'DELIRIUM_ORB',
			BULKY_DELIRIUM_ORBS.generateDeliriumOrbNameFromType,
	 ...
	 */
	function isScarab(obj: any): obj is BazaarDeliriumOrb {
		return (
			obj &&
			'type' in obj &&
