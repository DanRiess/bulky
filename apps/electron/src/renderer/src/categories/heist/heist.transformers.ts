...
function generateTypeFromPoeItem(poeItem: PoeItem): HeistType | undefined {
	...
	if (poeItem.baseType.includes('Contract')) {
		...
		const typeProp = properties.find(prop => prop.name === 'Requires {1} (Level {0})')
		...
		const escapeRoutes = properties.find(prop => prop.name === 'Escape Routes Revealed')
		const rewardRooms = properties.find(prop => prop.name === 'Reward Rooms Revealed')

		if (!wings || !escapeRoutes || !rewardRooms) return undefined

		const [revealedWings, totalWings] = wings.values[0][0].split('/').map(x => +x)
		const [revealedEscapeRoutes, totalEscapeRoutes] = escapeRoutes.values[0][0].split('/').map(x => +x)
		const [revealedRewardRooms, totalRewardRooms] = rewardRooms.values[0][0].split('/').map(x => +x)
...
function generateShopItemFromPoeItem(poeItem: PoeItem, itemOverrides: Ref<BulkyItemOverrideRecord>): ShopHeistItem | undefined {
