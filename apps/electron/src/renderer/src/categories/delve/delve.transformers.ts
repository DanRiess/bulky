...
export const BULKY_DELVE = {
	...
	generateDelveItemNameFromType,
	generateTier,
	generateDelveItemFromPoeItem,
...
function generateDelveItemFromPoeItem(
	poeItem: PoeItem,
...
function generateBazaarItemFromDto(item: BulkyBazaarItemDto): BazaarDelveItem {
	...
	return {
		...
		type,
		tier: '0',
		name: generateDelveItemNameFromType(type),
		quantity: item.qnt,
 ...
 */
function generateDelveItemNameFromType(type: DelveType) {
	return type
		.split('_')
		.map(t => capitalize(t))
		.join(' ')
