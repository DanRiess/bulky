...
export const useEssenceOfferStore = defineStore("essenceOfferStore", () => {
  ...
  function putOffer(dto: BulkyBazaarOfferDto) {
    ...
    const items = dto.items
      .map(
        (item) =>
          BULKY_FACTORY.generateBazaarItemFromDto(
            "ESSENCE",
            item
          ) as BazaarEssence
      )
      .filter(notEmpty);
    if (!items || !multiplier || !fullPrice || !ign || !league || !chaosPerDiv)
      return;
...
if (import.meta.hot) {
  import.meta.hot.accept(
    acceptHMRUpdate(useEssenceOfferStore, import.meta.hot)
  );
}
