...
import { EssenceFilter, EssenceFilterField } from './essence.types'
import { BulkyFilter } from '@shared/types/bulky.types'

export const useEssenceFilterStore = defineStore('essenceFilterStore', () => {
	const filters = ref<Map<EssenceFilter['uuid'], EssenceFilter>>(new Map())
	const currentFilterId = ref<EssenceFilter['uuid']>()
	...
	function createNewFilter() {
		const uuid = BULKY_UUID.generateTypedUuid<BulkyFilter<EssenceFilterField>>()
	 ...
	 * Add a filter field.
	 */
	function addFilterField(uuid: EssenceFilter['uuid']) {
	 ...
	 */
	function removeFilterField(uuid: EssenceFilter['uuid'], idx: number) {
		const filter = filters.value.get(uuid)
		if (!filter) return
