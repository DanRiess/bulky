 ...
 */
function generateTypeFromBaseType(
  baseType: string
...
function generateShopItemFromPoeItem(
  ...
  return {
    type,
    tier,
    name: type.match(/logbook/i)
      ? `${poeItem.baseType} (${generateNameFromTier(tier)})`
      : poeItem.baseType,
    icon: poeItem.icon,
...
): BazaarExpeditionItem | undefined {
...
function generateNameFromType(type: ExpeditionType) {
  return type
    .split("_")
    .map((t) => {
      const word = capitalize(t);
      return word === "Logbook" || word === "Contract" ? `${word}:` : word;
    })
    .join(" ");
}
...
function getPerItemAttributes(
  item: PoeItem
): LogbookPerItemAttributes | undefined {
  ...
  const mappedMods = logbookMods
    .map((mod) => {
      ...
      const faction = EXPEDITION_FACTION[index] as
        | ExpeditionFaction
        | undefined;
      if (!faction) return;
