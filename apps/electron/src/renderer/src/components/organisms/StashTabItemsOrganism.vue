<template>
  <div
    class="o-stash-tab-items flow animated-gradient-background"
    data-b-override
  >
    ...
    <div class="total-value">
      <PriceAtom
        ...
        :label="
          category === 'EXPEDITION'
            ? 'Total Value (excluding logbooks):'
            : 'Total Value:'
        "
      />
      ...
      <template v-else-if="operation === 'edit'">
        <ButtonAtom background-color="dark" @click="syncChanges"
          >Sync Changes</ButtonAtom
        >
      </template>
    </div>
  </div>
...
const currentStashListComponent = computed(() => {
  if (props.category === "MAP") return StashItemListMoleculeMap;
    ...
    return StashItemListMoleculeExpedition;
...
const { filteredItemsByCategory, updateItemsByStash } =
  usePoeItems(selectedStashTabs);
const categoryFilteredItemsByStash = filteredItemsByCategory(
  () => props.category
);
...
const { items, sortItems } = useBulkyItems(
  categoryFilteredItemsByStash,
  prices,
  itemOverrides,
  () => props.category
);
const { filteredItemRecord } = useFilterShopItems(items, filterModel);
console.log({ filteredItemRecord });

// GETTERS
const chaosValue = useAggregateItemPrice(
  filteredItemRecord,
  () => props.offerMultiplier ?? 1
);
