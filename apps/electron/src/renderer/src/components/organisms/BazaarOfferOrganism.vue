		...
		: filteredItems.value.reduce((prev, curr) => {
				try {
					...
					if (curr.category === 'MAP_8_MOD') {
						const perItemAttributes = BULKY_MAPS.filterIndividual8ModItemsBySubtype(curr, props.filter)
...
async function sendMessage() {
	const message = generateWhisperMessage(props.offer, props.filter, filteredItems, filteredPrice, props.priceComputeFn)
