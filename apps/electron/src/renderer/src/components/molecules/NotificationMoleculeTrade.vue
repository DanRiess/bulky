<template>
  <div
    class="m-notification-regex animated-gradient-background"
    data-b-override
  >
    <div class="message">
      <p>
      ...
      <p>
        League:
        <span
          :class="{ error: notification.league !== configStore.config.league }"
          >{{ notification.league }}</span
        >
      </p>
      ...
      <PriceAtom
        ...
        v-if="
          ...
          (mtn.trades.length > 0 || mtn.trades[0] === 'Full Offer')
        "
      />
      <p v-else-if="typeof totalPrice === 'number'">
        Full Price: {{ totalPrice }} c.
        <span class="error">(League mismatch: incorrect chaos/div ratio!)</span>
      </p>
    </div>
    <div class="buttons">
...
async function chatBoxAction(command: "invite" | "tradewith" | "kick") {
  ...
  if (command === "kick") {
    ...
    await request.exec(
      `@${props.notification.ign} Thank you for the trade. Have a nice day!`
    );
