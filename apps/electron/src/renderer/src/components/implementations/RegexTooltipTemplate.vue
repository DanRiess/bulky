  ...
  <div class="t-regex-tooltip" v-if="conformedPrices">
    <div class="regex-option avoid">
      <SvgIconAtom
        name="listRemove"
        :color="
          conformedPrices.avoid !== undefined
            ? 'var(--color-success)'
            : 'var(--color-error)'
        "
      />
      ...
      <div class="price">
        ...
        <img
          v-if="conformedPrices.avoid !== undefined"
    ...
    <div class="regex-option wanted">
      <SvgIconAtom
        name="listAdd"
        :color="
          conformedPrices.wanted !== undefined
            ? 'var(--color-success)'
            : 'var(--color-error)'
        "
      />
      ...
      <div class="price">
        ...
        <img
          v-if="conformedPrices.wanted !== undefined"
          src="/src/assets/png-icons/currency-chaos.png"
          height="24"
          width="24"
          decoding="async"
    ...
    <template v-if="conformedPrices.quantity">
      <div
        class="regex-option quantity"
        v-for="option in conformedPrices.quantity"
      >
        <SvgIconAtom
          name="quantity"
          :color="option ? 'var(--color-success)' : 'var(--color-error)'"
        />
    ...
    <template v-if="conformedPrices.packsize">
      <div
        class="regex-option packsize"
        v-for="option in conformedPrices.packsize"
      >
...
const conformedPrices = computed(() => {
  ...
  if (
    props.prices.avoidRegex === undefined ||
    typeof props.prices.avoidRegex === "number"
  ) {
  ...
  if (
    props.prices.wantedRegex === undefined ||
    typeof props.prices.wantedRegex === "number"
  ) {
  ...
  if (props.prices.quantityRegex) {
    for (const maybeFragment of props.prices.quantityRegex) {
      if ("available" in maybeFragment) {
        ...
        quantity !== undefined
          ? quantity.push(maybeFragment.addedPrice)
          : (quantity = [maybeFragment.addedPrice]);
      } else {
        quantity !== undefined
          ? quantity.push(maybeFragment)
          : (quantity = [maybeFragment]);
      }
    }
  }
  ...
  if (props.prices.packsizeRegex) {
    for (const maybeFragment of props.prices.packsizeRegex) {
      if ("available" in maybeFragment) {
        if (maybeFragment.available === false) continue;
        packsize !== undefined
          ? packsize.push(maybeFragment.addedPrice)
          : (packsize = [maybeFragment.addedPrice]);
      } else {
        packsize !== undefined
          ? packsize.push(maybeFragment)
          : (packsize = [maybeFragment]);
      }
    }
  }
