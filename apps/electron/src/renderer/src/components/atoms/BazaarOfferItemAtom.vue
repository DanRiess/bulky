<template>
  <li class="a-bazaar-offer-item">
    <div class="generic-item-data">
      ...
      <div class="stock" v-if="!filter.fullBuyout">
        x{{ formatNumber(item.computedQuantity) }}
      </div>
      <div class="quantity">
        {{
    ...
    >
      ...
      <SvgIconAtom
        name="listAdd"
        :color="
          item.regex.wantedRegex ? 'var(--color-success)' : 'var(--color-error)'
        "
      />
      <SvgIconAtom
        name="quantity"
        :color="
          item.regex.quantityRegex
            ? 'var(--color-success)'
            : 'var(--color-error)'
        "
      />
...
// STATE
const filterField = props.filter.fields.find(
  (field) => field.type === props.item.type && field.tier === props.item.tier
);
...
const gridColumn = computed(() =>
  props.filter.fullBuyout ? "span 5" : "span 6"
);
