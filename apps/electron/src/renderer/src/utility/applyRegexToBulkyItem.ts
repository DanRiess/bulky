 ...
 */
export function applyRegexToBulkyItem(
  ...
  if (
    ...
    (regexes.avoid.length > 0 && !item.regex?.avoidRegex === undefined) ||
    (regexes.wanted.length > 0 && !item.regex?.wantedRegex === undefined)
  ) {
    return 0;
  }
  ...
  return item.perItemAttributes.reduce((prev, curr) => {
    ...
    if (regexes.quantity.length > 0 && curr.properties?.itemQuantity) {
      if (
        !`item quantity: ${curr.properties.itemQuantity}%`.match(
          regexes.quantity[0]
        )
      ) {
        return prev;
    ...
    if (regexes.packSize.length > 0 && curr.properties?.packSize) {
      if (
        !`pack size: ${curr.properties.packSize}%`.match(regexes.packSize[0])
      ) {
        return prev;
      }
    }
