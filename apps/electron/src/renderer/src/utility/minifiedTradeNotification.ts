...
export function generateMinifiedTradeNotification(
  ...
  const typeToIdxMap = BULKY_FACTORY.getNameToIdxTypeMap(
    category,
    secondaryTypeOption
  );
    ...
    : // Buyout case for individual items.
      toValue(items)
        .map((item) => {
          ...
          //   category === "FRAGMENT" && filter.fullSets;
          let secondaryTypeOption = 0;
          if (category === "FRAGMENT") {
            secondaryTypeOption = filter.fullSets ? 1 : 0;
          ...
          const filterField = filter.fields.find(
            (field) =>
              field.type === adjustedItem.type &&
              field.tier === adjustedItem.tier
          );
          const quantity =
          ...
          dv.setUint8(2, secondaryTypeOption);
...
export function decodeMinifiedTradeNotification(mtn: string): DecodedMTN {
  ...
  if (
    !categoryIdxString ||
    typeof parseInt(categoryIdxString) !== "number" ||
    !blockString
  ) {
  ...
  const categoryDisplayValue = BULKY_TRANSFORM.stringToDisplayValue(
    category + categoryAddendum
  );
  ...
  if (!idxToTierMap) {
    throw new Error("No idx to tier maps");
  }
  ...
  const trades = blockString.split(":").map((block) => {
    try {
      ...
      const buffer = Uint8Array.from(atob(block), (c) =>
        c.charCodeAt(0)
      ).buffer;
      ...
      if (typeIdx === 0xff && tierIdx === 0xf && quantity === 0xffff) {
        ...
        const idxToTypeMap = BULKY_FACTORY.getIdxToNameTypeMap(
          category,
          !!secondaryTypeOption // Here, only used for fragments (sets vs frags), so boolean is fine.
        );
        ...
        return `${quantity} x ${BULKY_FACTORY.getNameFromTypeAndTier(
          category,
          item,
          secondaryTypeOption
        )} for ${Math.round(price * quantity * 10) / 10}c`;
      }
    } catch (e) {
