...
export function filterOffer(
  ...
  if (
    filter.multiplier &&
    filter.multiplier < 2 &&
    filter.multiplier < offer.multiplier
  )
    return null;
  ...
  for (let i = 0; i < computedFilterFields.length; ++i) {
    ...
    const item = offerClone.items.find(
    ...
    if (item) {
      ...
      if (requireFilteringIndividualItems(item, filter)) {
        // Currently, only 8 mod maps need this functionality.
        // If there will be another category like this in the future, add the function to the factory.
        const perItemAttributes = BULKY_MAPS.filterIndividual8ModItemsBySubtype(
          item,
          filter
        );

        // If we find the attributes, clone the offer item and replace the attributes.
        ...
        if (perItemAttributes) {
          ...
          if (BULKY_REGEX.regexAmount(regexes) > 0) {
            ...
            item.computedQuantity = perItemAttributes.length;
          }
        } else {
...
function filterFragmentSetOffer(
  filter: BulkyFilter,
  filterField: BulkyFilterField,
  offer: BulkyBazaarOffer
) {
  const necessaryItems: FragmentType[] | undefined =
    FRAGMENT_SET[filterField.type];
  if (!necessaryItems) {
    return {
  ...
  const allItemsAvailable = necessaryItems.every((necessaryItem) => {
    ...
    if (filter.alwaysMaxQuantity) {
      maxQuantityIfAlwaysMaxQuantity = Math.min(
        maxQuantityIfAlwaysMaxQuantity,
        itemInOffer.quantity
      );
    } else {
  ...
  const price = filter.alwaysMaxQuantity
    ? setPrice * maxQuantityIfAlwaysMaxQuantity
    : setPrice * filterField.quantity;
...
function requireFilteringIndividualItems(
  item: BulkyBazaarItem,
  filter: BulkyFilter
) {
  return (
    item.perItemAttributes !== undefined &&
    (filter.regex !== undefined ||
      filter.delirious !== undefined ||
      filter.originator !== undefined)
  );
}
