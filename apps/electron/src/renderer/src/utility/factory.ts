...
import {
  BEAST_TYPE,
  BEAST_TYPE_IDX_TO_NAME,
  BEAST_TYPE_NAME_TO_IDX,
} from "@web/categories/bestiary/bestiary.const";
import { BULKY_BESTIARY } from "@web/categories/bestiary/bestiary.transformers";
import { BeastTier } from "@web/categories/bestiary/bestiary.types";
...
import {
  CATALYST_TYPE,
  CATALYST_TYPE_IDX_TO_NAME,
  CATALYST_TYPE_NAME_TO_IDX,
} from "@web/categories/catalyst/catalyst.const";
...
import {
  CURRENCY_TYPE,
  CURRENCY_TYPE_IDX_TO_NAME,
  CURRENCY_TYPE_NAME_TO_IDX,
} from "@web/categories/currency/currency.const";
...
import {
  DELVE_TYPE,
  DELVE_TYPE_IDX_TO_NAME,
  DELVE_TYPE_NAME_TO_IDX,
} from "@web/categories/delve/delve.const";
...
import {
  ...
  SCARAB_TYPE_NAME_TO_IDX,
} from "@web/categories/scarab/scarab.const";
...
function getItemTypes(
  ...
  if (category === "ESSENCE") return getKeys(ESSENCE_TYPE);
    ...
    return getKeys(EXPEDITION_TYPE).filter((val) => val !== "LOGBOOK");
  else if (category === "FRAGMENT") {
 ...
 */
function getTypeFromPoeItem(
  item: PoeItem,
  category: Category
): BulkyShopItem["type"] | undefined {
  if (category === "ESSENCE")
    return BULKY_ESSENCES.generateTypeFromBaseType(item.baseType);
  else if (category === "SCARAB")
    return BULKY_SCARABS.generateTypeFromBaseType(item.baseType);
  else if (category === "DELIRIUM_ORB")
    return BULKY_DELIRIUM_ORBS.generateTypeFromBaseType(item.baseType);
  else if (category === "MAP" || category === "MAP_8_MOD")
    return BULKY_MAPS.generateTypeFromBaseType(item.baseType);
  else if (category === "BESTIARY")
    return BULKY_BESTIARY.generateTypeFromPoeItem(item);
  else if (category === "DELVE")
    return BULKY_DELVE.generateTypeFromBaseType(item.baseType);
  else if (category === "CATALYST")
    return BULKY_CATALYSTS.generateTypeFromBaseType(item.baseType);
  else if (category === "CURRENCY")
    return BULKY_CURRENCY.generateTypeFromBaseType(item.baseType);
  else if (category === "HEIST")
    return BULKY_HEIST.generateTypeFromPoeItem(item);
  else if (category === "EXPEDITION")
    return BULKY_EXPEDITION.generateTypeFromBaseType(item.baseType);
  else if (category === "FRAGMENT")
    return BULKY_FRAGMENT.generateTypeFromBaseType(item.baseType);
  else return undefined;
}
 ...
 */
function getTierFromPoeItem(
  item: PoeItem,
  category: Category
): BulkyShopItem["tier"] | undefined {
  if (category === "ESSENCE")
    return BULKY_ESSENCES.generateTierFromBaseType(item.baseType);
  else if (category === "SCARAB") return BULKY_SCARABS.generateTier();
  else if (category === "DELIRIUM_ORB")
    return BULKY_DELIRIUM_ORBS.generateTier();
  else if (category === "MAP" || category === "MAP_8_MOD")
    return BULKY_MAPS.generateTierFromProperty(item.properties);
    ...
    return BULKY_HEIST.generateTierFromItemLevel(item.ilvl);
  else if (category === "EXPEDITION")
    return BULKY_EXPEDITION.generateTierFromProperty(item.ilvl);
...
function getNameFromTypeAndTier(
  ...
  if (category === "ESSENCE" && useEssenceOfferStore().isEssence(item))
    ...
    return BULKY_SCARABS.generateNameFromType(item.type);
  else if (
    category === "DELIRIUM_ORB" &&
    useDeliriumOrbOfferStore().isDeliriumOrb(item)
  )
    return BULKY_DELIRIUM_ORBS.generateNameFromType(item.type);
  else if (category === "MAP" && useNormalMapOfferStore().isNormalMap(item))
    return BULKY_MAPS.generateNameFromType(item.type);
    ...
    return BULKY_DELVE.generateNameFromType(item.type);
  else if (category === "CATALYST" && useCatalystOfferStore().isCatalyst(item))
    return BULKY_CATALYSTS.generateNameFromType(item.type);
  else if (
    category === "CURRENCY" &&
    useCurrencyOfferStore().isCurrencyItem(item)
  )
    return BULKY_CURRENCY.generateNameFromType(item.type);
  else if (category === "HEIST" && useHeistOfferStore().isHeistItem(item))
    // return BULKY_HEIST.generateNameFromType(item.type);
    return BULKY_HEIST.getNameFromTypeAndTier(item.type, item.tier);
  else if (
    category === "EXPEDITION" &&
    useExpeditionOfferStore().isExpeditionItem(item)
  )
    return BULKY_EXPEDITION.getNameFromTypeAndTier(item.type, item.tier);
...
function generateBazaarItemFromDto(
  ...
  if (category === "ESSENCE")
    return BULKY_ESSENCES.generateBazaarItemFromDto(item);
  else if (category === "SCARAB")
    return BULKY_SCARABS.generateBazaarItemFromDto(item);
  else if (category === "DELIRIUM_ORB")
    return BULKY_DELIRIUM_ORBS.generateBazaarItemFromDto(item);
  else if (category === "MAP")
    return BULKY_MAPS.generateBazaarItemFromDto(item);
  // does not need a Map_8_MOD implementation because the dto is different
  else if (category === "BESTIARY")
    return BULKY_BESTIARY.generateBazaarItemFromDto(item);
  else if (category === "DELVE")
    return BULKY_DELVE.generateBazaarItemFromDto(item);
  else if (category === "CATALYST")
    return BULKY_CATALYSTS.generateBazaarItemFromDto(item);
  else if (category === "CURRENCY")
    return BULKY_CURRENCY.generateBazaarItemFromDto(item);
  else if (category === "HEIST")
    return BULKY_HEIST.generateBazaarItemFromDto(item);
  else if (category === "EXPEDITION")
    return BULKY_EXPEDITION.generateBazaarItemFromDto(item);
  else if (category === "FRAGMENT")
    return BULKY_FRAGMENT.generateBazaarItemFromDto(item);
  else return undefined;
}
...
function generateBulkyItemFromPoeItem(
  ...
  if (category === "ESSENCE")
    return BULKY_ESSENCES.generateShopItemFromPoeItem(
      item,
      prices,
      itemOverrides
    );
  else if (category === "SCARAB")
    return BULKY_SCARABS.generateShopItemFromPoeItem(
      item,
      prices,
      itemOverrides
    );
  else if (category === "DELIRIUM_ORB")
    return BULKY_DELIRIUM_ORBS.generateShopItemFromPoeItem(
      item,
      prices,
      itemOverrides
    );
  else if (category === "MAP")
    return BULKY_MAPS.generateShopItemFromPoeItem(item, prices, itemOverrides);
  else if (category === "MAP_8_MOD")
    return BULKY_MAPS.generateShopMap8ModItemFromPoeItem(item, itemOverrides);
  else if (category === "BESTIARY")
    return BULKY_BESTIARY.generateShopItemFromPoeItem(
      item,
      prices,
      itemOverrides
    );
  else if (category === "DELVE")
    return BULKY_DELVE.generateShopItemFromPoeItem(item, prices, itemOverrides);
  else if (category === "CATALYST")
    return BULKY_CATALYSTS.generateShopItemFromPoeItem(
      item,
      prices,
      itemOverrides
    );
  else if (category === "CURRENCY")
    return BULKY_CURRENCY.generateShopItemFromPoeItem(
      item,
      prices,
      itemOverrides
    );
  else if (category === "HEIST")
    return BULKY_HEIST.generateShopItemFromPoeItem(item, itemOverrides);
  else if (category === "EXPEDITION")
    return BULKY_EXPEDITION.generateShopItemFromPoeItem(
      item,
      prices,
      itemOverrides
    );
  else if (category === "FRAGMENT")
    return BULKY_FRAGMENT.generateShopItemFromPoeItem(
      item,
      prices,
      itemOverrides
    );
...
function getPerItemAttributes(
  ...
  if (category === "MAP_8_MOD") return BULKY_MAPS.getPerItemAttributes(item);
    ...
    return BULKY_EXPEDITION.getPerItemAttributes(item);
  return undefined;
}
